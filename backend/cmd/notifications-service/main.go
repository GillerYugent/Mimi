...
import (
	...
	"github.com/gilleryugent/mimi-backend/services/notifications"
	"github.com/gorilla/mux"
	"github.com/rs/cors"
)

func main() {
	cfg, err := config.Load("notifications-service")
	if err != nil {
		panic(err)
	}
	...
	if err := db.Migrate(rootCtx, pool, "/migrations/notifications"); err != nil {
		logger.Error("migrate", "err", err)
		os.Exit(1)
	}

	rdb, err := cache.Connect(rootCtx, cfg.RedisURL)
	if err != nil {
	...
	handler := notifications.NewHandler(svc, cfg.JWTSecret)
	...
	go func() {
		logger.Info("notifications-service listening", "port", cfg.Port)
	...
	rootCancel() // останавливаем consumer Pub/Sub
