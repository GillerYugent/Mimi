...
import (
	...
	"github.com/gilleryugent/mimi-backend/services/tasks"
	"github.com/gorilla/mux"
	"github.com/rs/cors"
)

func main() {
	cfg, err := config.Load("tasks-service")
	if err != nil {
		panic(err)
	}
	...
	if err := db.Migrate(ctx, pool, "/migrations/tasks"); err != nil {
	...
	repo := tasks.NewRepo(pool)
	svc := tasks.NewService(repo, rdb)
	handler := tasks.NewHandler(svc, cfg.JWTSecret)
	...
	go func() {
		logger.Info("tasks-service listening", "port", cfg.Port)
