...
import (
	...
	"github.com/gilleryugent/mimi-backend/services/auth"
...
func main() {
	...
	if err := db.Migrate(ctx, pool, "/migrations/auth"); err != nil {
	...
	svc := auth.NewService(repo, rdb, cfg)
	handler := auth.NewHandler(svc, cfg.JWTSecret)
