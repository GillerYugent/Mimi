...
import (
	...
	"github.com/gilleryugent/mimi-backend/services/git"
	"github.com/gorilla/mux"
	"github.com/rs/cors"
)

func main() {
	cfg, err := config.Load("git-service")
	if err != nil {
		panic(err)
	}
	...
	if err := db.Migrate(ctx, pool, "/migrations/git"); err != nil {
	...
	repo := git.NewRepo(pool)
	providers := gitprovider.NewRegistry()
	svc := git.NewService(repo, rdb, providers)
	handler := git.NewHandler(svc, cfg.JWTSecret)
	...
	go func() {
		logger.Info("git-service listening", "port", cfg.Port)
