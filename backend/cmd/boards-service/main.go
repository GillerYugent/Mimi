...
import (
	...
	"github.com/gilleryugent/mimi-backend/services/boards"
	"github.com/gorilla/mux"
	"github.com/rs/cors"
)

func main() {
	cfg, err := config.Load("boards-service")
	if err != nil {
		panic(err)
	}
	...
	if err := db.Migrate(ctx, pool, "/migrations/boards"); err != nil {
	...
	repo := boards.NewRepo(pool)
	svc := boards.NewService(repo)
	handler := boards.NewHandler(svc, cfg.JWTSecret)
	...
	go func() {
		logger.Info("boards-service listening", "port", cfg.Port)
